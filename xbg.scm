 ...
 (let* ((w (car dim))
  ...
  (if (= (length weather) 8)
   (let* ((wcondicon (list-ref weather 0))
   	  ...
   	  (curdewp (list-ref weather 3))
	  (cloudp (list-ref weather 4))
	  (rainp (list-ref weather 5))
	  (windspd (list-ref weather 6))
	  (winddir (list-ref weather 7))
	 )
    (if (> (car cloudp) 0)
     (let* ((cp (car cloudp))
    ...
    (let* ((maxt (apply max exttemp))
	   ...
	   (xw (* 4 bord))
	   (xs (- xe xw))
	   (xi (lambda (i n) (let ((p (- 1 (/ i n))))
			       (- xe (* (* p p) xw)))))
           (lext (length exttemp))
	   (drawext (lambda (p i l n)
		      (if (null? l) n
			(let ((t (car l)))
	   ...
	   (templine (car (gimp-layer-new img (succ (- w xs)) (* 2 bord) RGBA-IMAGE "temp" 75 DIFFERENCE-MODE)))
	   (tempvect (car (gimp-vectors-new img "temp")))
	   (dewpvect (car (gimp-vectors-new img "dewp")))
	   (popvect (car (gimp-vectors-new img "pop")))
	   (makedata
	     (lambda (lb ub l)
	       (let* ((n (length l))
		      (v (make-vector (* 6 n)))
		 ...
		 (f 0 l)
		 v)))
	   (tempdata (makedata mint maxt curtemp))
	   (dewpdata (makedata mint maxt curdewp))
	   (popdata (makedata 0 100 rainp))
	   (pop (car rainp))
	   (popside (<= (tu curt) 0.5))
           (wcondi (if (not (null? wcondicon)) (assoc (car wcondicon)
		    '((skc . "sun")		(nskc . "sun")
		      (few . "sun")	  	(nfew . "sun")
     ...
     (if (not (null? wcondi))
      (let* ((wcond (add-img (cdr wcondi)))
       ...
       (gimp-layer-set-offsets wcond (- xs (car wcsize)) 0)
	...
	(gimp-layer-set-opacity wcond (cadr wcondicon)))
     ...
     (if (> (length curtemp) 1) 
      (begin
       ...
       (gimp-layer-set-offsets templine xs 0)
       (gimp-context-set-brush "2. Hardness 100")
       (gimp-context-set-brush-size 1.0)
       (gimp-context-set-paint-method "gimp-paintbrush")

       (gimp-image-add-vectors img popvect -1)
       (let ((popstr (car (gimp-vectors-stroke-new-from-points popvect 0 (vector-length popdata) popdata FALSE))))
	 (gimp-vectors-bezier-stroke-lineto popvect popstr xe bord)
	 (gimp-vectors-bezier-stroke-lineto popvect popstr xs bord)
	 (gimp-vectors-stroke-close popvect popstr))
       (gimp-image-select-item img CHANNEL-OP-REPLACE popvect)
       (gimp-context-set-foreground '(128 255 255))
       (gimp-context-set-opacity 50)
       (gimp-drawable-edit-fill templine FILL-FOREGROUND)
       (gimp-selection-none img)
       (gimp-context-set-opacity 100)

       (gimp-image-add-vectors img dewpvect -1)
       (gimp-vectors-stroke-new-from-points dewpvect 0 (vector-length dewpdata) dewpdata FALSE)
       (gimp-context-set-foreground '(255 255 128))
       (gimp-edit-stroke-vectors templine dewpvect)
