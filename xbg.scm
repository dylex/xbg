  ...
  (if (= (length weather) 8)
   (let* ((wcondicon (list-ref weather 0))
    ...
    (let* ((maxt (apply max exttemp))
	   ...
	   (nl (pred (length xl)))
	   (px (cadr xl))
	   (templine (car (gimp-layer-new img (succ (- w px)) (* 2 bord) RGBA-IMAGE "temp" 75 DIFFERENCE-MODE)))
	     ...
	     (lambda (lb ub l)
	       (let* ((n (length l))
		      ...
		      (f (lambda (i l)
			   (if (< i n)
			     (let ((j (* 6 i))
				   (x (imap i 0 (pred n) (+ px 2) (- w 2)))
				   (y (imap (car l) lb ub (- bord 2) 2)))
			       (vector-set! v (+ j 0) x)
			       (vector-set! v (+ j 1) y)
			       (vector-set! v (+ j 2) x)
			       (vector-set! v (+ j 3) y)
	...
	(gimp-layer-set-opacity wcond (cadr wcondicon)))
     ...
     (if (> (length curtemp) 1) 
      (begin
       ...
       (gimp-vectors-stroke-new-from-points popvect 0 (vector-length popdata) popdata FALSE)
       (gimp-context-set-foreground '(255 128 255))
       (gimp-edit-stroke-vectors templine popvect)
