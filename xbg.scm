  ...
  (if (= (length weather) 8)
   (let* ((wcondicon (list-ref weather 0))
    ...
    (let* ((maxt (apply max exttemp))
	   ...
	   (datalen (* 6 (min (length curtemp) (length curdewp))))
	   (tempdata (make-vector datalen))
	   (maketemp (lambda (i t)
		      (if (< i datalen)
		       (let ((x (imap i 0 (- datalen 6) (+ px 2) (- w 2)))
	...
	(gimp-layer-set-opacity wcond (cadr wcondicon)))
     ...
     (if (> (length curtemp) 1) 
      (begin
       ...
       (gimp-vectors-stroke-new-from-points tempvect 0 datalen tempdata FALSE)
       (gimp-context-set-foreground '(255 255 255))
       (gimp-edit-stroke-vectors templine tempvect)
       (maketemp 0 curdewp)
       (gimp-image-add-vectors img dewpvect -1)
       (gimp-vectors-stroke-new-from-points dewpvect 0 datalen tempdata FALSE)
